...
class Cassette
{
    ...
    public function playback(Request $request): ?Response
    {
        foreach ($this->storage as $recording) {
            ...
            // If matchers are not defined (because the cassette has been generated by PHP-VCR v1, let's use default matchers)
            $matcher = $this->getRequestMatcher($recording['matchers'] ?? $this->config->getEnabledRequestMatcherNames());

            if ($matcher->match($storedRequest, $request)) {
    ...
    public function record(Request $request, Response $response): void
    {
        ...
        $recording = array(
            ...
            'matchers' => $this->config->getEnabledRequestMatcherNames()
        );
     ...
     *
     * @param string[] $matchers The list of matchers to use.
